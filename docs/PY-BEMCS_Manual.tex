...
\(E_{\mathrm{th}}\) is the sputter threshold, and
\(\kappa_{\mathrm{acc}}\) is the user acceleration factor.
When a cell exceeds the user damage threshold
\(D_{\mathrm{fail}}\), the cell is marked non-solid and the
Poisson operator is refactorised.
